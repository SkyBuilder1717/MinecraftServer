...
public class EntitySheep extends EntityAnimals {
    ...
    public boolean attackEntity(Entity var1, int var2) {
        if(!this.sheared && var1 instanceof EntityLiving) {
            this.sheared = true;
            int var3 = 1 + worldObj.rand.nextInt(3);

            for(int var4 = 0; var4 < var3; ++var4) {
                EntityItem var5 = this.dropItemWithOffset(Block.cloth.blockID, 1, 1.0F);
                var5.motionY += worldObj.rand.nextFloat() * 0.05F;
                var5.motionX += (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.1F;
                var5.motionZ += (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.1F;
            }
        }
